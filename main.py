...
SCENARIO_KEYWORDS = ["Belén Andrés", "campo N 3"]
DIVISION_NAME = "Completa"
WEEKS_TO_TRY = 3
DESIRED_TIME_SLOT = "08:00 PM - 09:00 PM"
...
async def launch_program(pw_context_mng, headless: bool = HEADLESS, slow_mo: int | None = SLOW_MO):
...
async def fill_reservation_form(page, selections: dict = SCENARIO_FILTERS):
...
async def select_division(page, division_name: str = DIVISION_NAME):
...
async def select_time_slot(page,max_weeks_to_try: int = WEEKS_TO_TRY,desired_time_slot: str = DESIRED_TIME_SLOT):
...
async def add_participant(page, guest_numdoc: str = GUEST_NUMDOC):
